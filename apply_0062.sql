-- Apply migration 0062 to production and record it, exactly as scripts/migrate.py would.
-- Read-only function, no data changed. Paste the whole thing into Railway's Postgres Data tab.
begin;
...
insert into app.schema_migrations (filename, checksum) values ('0062_ledger_balances_as_of.sql', 'ca6dcd86ec0af77fd5798f33d846d8e5b20460d1fa03006facec16a026b92808') on conflict (filename) do nothing;

commit;

-- Check (should return 1 row, true):
select exists (select 1 from pg_proc p join pg_namespace n on n.oid = p.pronamespace where n.nspname = 'ledger' and p.proname = 'balances_as_of') as m0062;

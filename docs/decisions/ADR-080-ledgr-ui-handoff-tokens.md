...
7. **Recovery and dashboard follow-ups.** `POST /v1/auth/recover` (IAM-018) and a web forgot-password screen exist now. The dashboard returns `receipts_to_review` (Review nav count), `customer_name` and `amount`: a draft receipt's gross, or an overdue invoice's outstanding balance from the same open-items query the ageing report reads (after payments and credits, not the invoice total). Still open, because each needs a ledger-API change: a date-bounded balance for a monthly `cash_history` (and so deltas), and a confirmed statutory BTW filing rule for a due date. A draft invoice carries no amount: nothing is owed on it yet.
